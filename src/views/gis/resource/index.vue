...
import { getResourceList, getResource, getResourceListByAOI, getAOIBorderList, getAOIInfo } from "@/api/gis"
  ...
  polygon.on("click", () => {
    ...
    if (mode[1] === 0) {
      if (extData.showStatus === 0) {
        addResourceByAOI(AMap, labelsLayer, extData.aoiId)
...
const addResources = (AMap, labelsLayer, sw, ne) => {
  getResourceList(sw.lng, ne.lng, sw.lat, ne.lat).then(function (response) {
    showResources(AMap, labelsLayer, response)
  })
}

const addResourceByAOI = (AMap, labelsLayer, aoiId) => {
  getResourceListByAOI(aoiId).then(function (response) {
    showResources(AMap, labelsLayer, response)
  })
}

const showResources = (AMap, labelsLayer, resourceList) => {
  labelsLayer.clear()
  // 普通点
  ...
  for (let i = 0; i < resourceList.length; i++) {
    const marker = new AMap.LabelMarker({
      position: [resourceList[i].longitude, resourceList[i].latitude],
      icon: {
        type: "image",
    ...
    marker.on("mouseover", function (e) {
      ...
      let resourceName = "未知名称"
      getResource(position[0], position[1]).then(function (response) {
        if (response != null) {
...
const clickAOI = (AMap, aoiId) => {
  ...
  getAOIInfo(aoiId).then((response) => {
    ...
    content.push("资源点数量：" + String(response.resourceNum) + "</div>")
  ...
  AMapLoader.load({
    ...
    .then((AMap) => {
      ...
      map.on("moveend", function () {
        ...
        if (mode[1] === 1) {
          addResources(AMap, labelsLayer, sw, ne)
        }
      })
    })
    .catch((e) => {
